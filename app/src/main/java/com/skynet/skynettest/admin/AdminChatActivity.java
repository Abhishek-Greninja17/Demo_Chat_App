...
public class AdminChatActivity extends AppCompatActivity {
    ...
    private String senderId, senderName, receiverId, receiverName, email, receiverImage;
    private FirebaseAuth firebaseAuth;
    private DatabaseReference dbRef;
    private ImageView back, sendButton, selectAttachment, deleteAll;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        back = findViewById(R.id.icon_back_chat_employee);
        deleteAll = findViewById(R.id.delete_all);
        userLogo = findViewById(R.id.chat_user_logo);
        name = findViewById(R.id.chat_user_name);
        online = findViewById(R.id.online);
        inputMsg = findViewById(R.id.input_chat_message);
        sendButton = findViewById(R.id.send_message_button);
        selectAttachment = findViewById(R.id.select_chat_attachment);

        loadingBar = new ProgressDialog(this);
        userMessageList = findViewById(R.id.chatRecyclerView);
        adminMessageAdapter = new AdminMessageAdapter(messagesList, this);
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        userMessageList.setLayoutManager(layoutManager);
        userMessageList.setAdapter(adminMessageAdapter);

        name.setText(receiverName);
        Glide.with(this).load(receiverImage).placeholder(R.drawable.user_profile_icon).into(userLogo);
        ...
        deleteAll.setOnClickListener(v -> new AlertDialog.Builder(AdminChatActivity.this)
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        dbRef.child("Employee Message").child(receiverId).child(senderId).removeValue()
                                .addOnCompleteListener(task12 -> dbRef.child("Admin Message").child(senderId)
                                        .child(receiverId).removeValue()
                                        .addOnCompleteListener(task1 -> {
                                            dbRef.child("Admin Message").child(receiverId).child(senderId)
                                                    .removeValue();
                                            Toast.makeText(AdminChatActivity.this,
                                                    "Chat delete successfully..", Toast.LENGTH_SHORT).show();
                                            finish();
                                        }));
                    }
                });
    ...
    private void retrieveChat() {
        messagesList.clear();
        dbRef.child("Admin Message").child(senderId).child(receiverId)
                .addChildEventListener(new ChildEventListener() {
                    ...
                    public void onChildAdded(@NonNull DataSnapshot snapshot, @Nullable String previousChildName) {
                        ...
                        Notification notification = new Notification.Builder(getApplicationContext()).setContentTitle("New message").setContentText(messageModel.getMessage()).
                                setContentTitle("New message").setSmallIcon(R.drawable.ic_speech_bubble).build();
...
//                        displayNotification();
                    ...
                    public void onChildRemoved(@NonNull DataSnapshot snapshot) {
                        MessageModel messageModel = snapshot.getValue(MessageModel.class);
                        messagesList.remove(messageModel);
                        adminMessageAdapter.notifyDataSetChanged();
                        userMessageList.smoothScrollToPosition(userMessageList.getAdapter().getItemCount());
                    }
